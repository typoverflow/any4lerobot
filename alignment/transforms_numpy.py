...
-----------
  ...
  frame names : R_a^b ("R_a_to_b") is the orientation of frame a expressed in b;
                ...
                (gripper) frame, w = world, c = the frame the model was trained in.
"""
...
def axis_alignment_matrix(x_to: str, y_to: str, z_to: str) -> np.ndarray:
...
def align_axis(R, p, R_world_align, R_gripper_align=None):
    ...
    R_aligned = np.matmul(Rw, _f32(R))
    if R_gripper_align is not None:
        R_aligned = np.matmul(R_aligned, np.swapaxes(_f32(R_gripper_align), -1, -2))
    p_aligned = _matvec(Rw, _f32(p))
    return R_aligned, p_aligned


# === SE(3) frame math (see design_of_state_and_action_space.md) ==================================
def gripper_delta_pose(R_cur: np.ndarray, p_cur: np.ndarray, R_tgt: np.ndarray, p_tgt: np.ndarray):
...
def world_delta_pose(R_cur: np.ndarray, p_cur: np.ndarray, R_tgt: np.ndarray, p_tgt: np.ndarray):
...
def change_delta_pose_frame(R_delta: np.ndarray, p_delta: np.ndarray, R_src_to_dst: np.ndarray):
